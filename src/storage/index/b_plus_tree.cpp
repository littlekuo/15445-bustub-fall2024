...
auto BPLUSTREE_TYPE::GetValue(const KeyType &key, std::vector<ValueType> *result) -> bool {
  ...
  Context ctx;
  auto guard_variant = FindLeafPage(key, &ctx, OperationType::kRead);
  if (std::holds_alternative<std::monostate>(guard_variant)) {
    return false;
  }
  ReadPageGuard guard = std::get<ReadPageGuard>(std::move(guard_variant));
...
auto BPLUSTREE_TYPE::Insert(const KeyType &key, const ValueType &value) -> bool {
  ...
  Context ctx;
  // 1 find leaf page
  WritePageGuard cur_guard = std::get<WritePageGuard>(FindLeafPage(key, &ctx, OperationType::kInsert));
  ...
  auto inserted = leaf_page->Insert(key, value, comparator_, redistributions);
  if (!inserted) {
    ctx.Drop();
    return false;
  }
...
void BPLUSTREE_TYPE::Remove(const KeyType &key) {
  // Declaration of context instance.
  Context ctx;
  auto guard_variant = FindLeafPage(key, &ctx, OperationType::kRemove);
  if (std::holds_alternative<std::monostate>(guard_variant)) {
    return;
  }
  WritePageGuard cur_guard = std::get<WritePageGuard>(std::move(guard_variant));
  auto leaf_page = cur_guard.AsMut<LeafPage>();
  auto removed = leaf_page->Remove(key, comparator_);
  if (!removed || !leaf_page->IsUnderflow()) {
    ctx.Drop();
    return;
  }
  if (ctx.IsRootPage(leaf_page->GetPageId())) {
    if (leaf_page->IsEmpty()) {
  ...
  for (auto page = cur_guard.As<BPlusTreePage>(); page->IsUnderflow();) {
    if (page->IsLeafPage()) {
      page_id = MergeOrRedistributeLeafPage(&cur_guard, &ctx);
    } else {
      if (ctx.IsRootPage(cur_guard.As<InternalPage>()->GetPageId())) {
        if (page->GetSize() <= 1) {
          ctx.header_page_.value().AsMut<BPlusTreeHeaderPage>()->root_page_id_ = page_id;
        }
        break;
      }
      // std::cout << "root: " << ctx.root_page_id_ << " internal page: " << cur_guard.GetPageId() << " size: " <<
      // page->GetSize() << std::endl;
...
auto BPLUSTREE_TYPE::Begin() -> INDEXITERATOR_TYPE {
  ...
  return INDEXITERATOR_TYPE(bpm_, 0, std::move(guard.value()));
}
...
auto BPLUSTREE_TYPE::Begin(const KeyType &key) -> INDEXITERATOR_TYPE {
  ...
  return INDEXITERATOR_TYPE(bpm_, index, std::move(guard));
}
...
auto BPLUSTREE_TYPE::MergeOrRedistributeLeafPage(WritePageGuard *page_guard, Context *ctx) -> page_id_t {
  ...
  // std::cout << "leaf_page_id: " << leaf_page->GetPageId() << "idx: " << idx << std::endl;
  if (idx < parent_page->GetSize() - 1) {
    ...
    if (right_sibling_page->IsAboveMinThreshold()) {
      ...
      leaf_page->SetKeyValueAt(leaf_page->GetSize() - 1, key, value);
    ...
    leaf_page->ChangeSizeBy(right_sibling_page->GetSize());
    for (int i = 0; i < right_sibling_page->GetSize(); i++) {
      leaf_page->SetKeyValueAt(old_size + i, right_sibling_page->KeyAt(i), right_sibling_page->ValueAt(i));
    }
  ...
  *page_guard = bpm_->WritePage(cur_page_id);
  ...
  if (left_sibling_page->IsAboveMinThreshold()) {
    auto key = left_sibling_page->KeyAt(left_sibling_page->GetSize() - 1);
    auto value = left_sibling_page->ValueAt(left_sibling_page->GetSize() - 1);
  ...
  left_sibling_page->ChangeSizeBy(leaf_page->GetSize());
  for (int i = 0; i < leaf_page->GetSize(); i++) {
    left_sibling_page->SetKeyValueAt(old_size + i, leaf_page->KeyAt(i), leaf_page->ValueAt(i));
  }
...
auto BPLUSTREE_TYPE::MergeOrRedistributeInternalPage(WritePageGuard *page_guard, Context *ctx) -> page_id_t {
  ...
  auto parent_page = parent_guard.AsMut<InternalPage>();
  auto idx = parent_page->ValueIndex(page_guard->GetPageId());
  // std::cout << "internal_page_id: " << internal_page->GetPageId() << "idx: " << idx << std::endl;
  // 1. right sibling
  if (idx < parent_page->GetSize() - 1) {
    WritePageGuard right_sibling_guard = bpm_->WritePage(parent_page->ValueAt(idx + 1));
